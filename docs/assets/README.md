# Partuno assets

This directory contains reusable branding and demonstration assets.

- `partuno-logo.png` is the full-resolution project logo.
- `partuno-plugin-icon.png` is a compact PNG for client or plugin upload fields
  with strict file-size limits.
- `demos/` contains the public demonstration derivatives:
  - `featured-recommendation.gif` and its two response stills.
  - `exact-offer-comparison.gif` and its response still.
  - `product-research.gif` and its response still.
  - `bom-risk-review.gif` and its two response stills.

The GIFs are shortened prompt-to-result captures that preserve the natural-
language prompt and the response scroll while removing the screen-recording
HUD. The original full-length MOV recordings remain outside the repository.
The captures were made August 11–12, 2026; provider inventory, pricing, and
lead-time values can change.

The Catalog search capture is retained locally as a technical validation asset
because its current prompt is tool/JSON-oriented rather than a representative
public user workflow. A future plain-English recording can be added here if
desired.

Do not add API keys, OAuth codes, bearer tokens, private URLs, customer data,
account identifiers, or reusable mutation tokens to this directory.
